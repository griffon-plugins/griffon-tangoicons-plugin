 ...
 */
public class TangoIconFactory extends AbstractFactory {
    @Override
    public Object newInstance(FactoryBuilderSupport builder, Object name, Object value, Map attributes) throws InstantiationException, IllegalAccessException {
        ...
        if (icon instanceof CharSequence) {
            return new TangoIcon(Tango.findByDescription(icon.toString()), size);
        } else if (icon instanceof Tango) {
            return new TangoIcon((Tango) icon, size);
        }

        throw new IllegalArgumentException(name + " cannot parse " + icon + " as a TangoIcon description.");
    }
}
